...
const application = mode === "application-unlock";
const unlocking = mode.endsWith("unlock");
// Bundled from src/db/index.ts by the parent before the isolation preload runs.
const pool = application
  ? (await import("../../tmp/e2e/isolation-application-db.mjs")).createDb(
      process.env.DATABASE_URL,
      1,
    ).pool
  : new Pool({ connectionString: process.env.DATABASE_URL, max: 1 });
const send = (event, fields = {}) => process.send?.({ event, ...fields });
const transportError = new Error("owned test socket disconnected: secret-sentinel");
...
if (application) assert.equal(pool.listenerCount("error"), 0);
else
...
try {
  ...
  assert.equal(errors, unlocking && !application ? 1 : 0);
  assert.equal(acquisitionFailures, unlocking ? 0 : 1);
  assert.equal(
  ...
  if (application) {
    assert.equal(process.exitCode, 1, "harness failure cannot exit green");
    await assert.rejects(pool.query("SELECT count(*) FROM account"), {
      message: "[e2e] database isolation pool failed",
    });
    send("harness-failure");
    await pool.end();
    ...
    assert.equal(
      active.listenerCount("error"),
      0,
      "no temporary admission listener leaks",
    );
    await active.query("SELECT count(*) FROM account");
    const fresh = active;
    active = undefined;
    ...
    await pool.end();
    assert.equal(
      fresh.listenerCount("error"),
      1,
      "only pg-pool's return listener remains",
    );
    assert.equal(pool.totalCount, 0);
    assert.equal(errors, unlocking ? 1 : 0);
    send("done");
  }
} catch {
  send("probe-failed");
