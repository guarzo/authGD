 ...
 *
 * It works on the string and never on a number. `numeric(20,2)` holds 18 whole
 * digits, so the largest amount this can be handed is `999999999999999999.99` —
 * `Number()` of it rounds three digits early and would render an ISK figure
 * that is not the one in the database. The trailing `.00` is deliberately kept:
 * every value in a column is formatted the same way, and dropping the fraction
 * only on whole amounts would ragged the decimal point of the one column that
 * most needs it lined up.
 *
