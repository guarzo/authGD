...
Before a report can be checked, preparation must establish a validated packet identity. If CLI preparation fails or manual invocation lacks a validated packet identity, return the no-identity abort diagnostic with exact `Bundle: unavailable`, a safe blocking reason, unreviewed inputs, and corrective action. Include no applicant findings, citations, or invented metadata. It is not submitted to `checkReport` and cannot count as a completed check. A model-aborted report tied to a successfully prepared packet instead uses the actual bundle identity and contains no citations.

The checker CLI emits a no-identity preparation diagnostic without opening the report when its bundle is unsafe or uninterpretable. A valid model report exits 0, an invalid input or report exits 1, and invalid invocation syntax exits 2. Diagnostics are fixed and do not echo report contents.
