...
import * as clock from "@/services/fleet-key-identity";
...
import { POST as beginRoute } from "@/app/api/fleet/v2/recovery-challenges/route";
import { POST as completeRoute } from "@/app/api/fleet/v2/recovery-challenges/[id]/complete/route";
import { GET as deviceRoute } from "@/app/api/fleet/v2/device/route";
...
async function challenge(candidate?: ReturnType<typeof fleetKeyPair>) {
  ...
  const input = initiationBody(keys);
  const req = request(PATH, input);
  const response = await beginRoute(req);
    ...
    createHash("sha256")
      .update(
        [
          ...
          "POST",
          PATH,
          req.headers.get("x-fleet-attempt"),
...
function complete(
  ...
  // Immutable recovery proof domain remains v1; API/attempt correlation is separate.
...
describe("actual key recovery routes", () => {
  it("returns only the terminal proven conflict outcome for real reconciled legacy aliases", async () => {
    ...
    const response = await complete(await challenge(keys));
    expect(response.status).toBe(200);
    expect(response.headers.get("cache-control")).toBe("no-store");
    expect(await response.json()).toEqual({ protocol: 2, result: "device_key_conflict" });
  ...
  it("requires strict initiation fields and refuses invalid proofs uniformly without allocating", async () => {
    ...
    for (const keys of [paired, fleetKeyPair()]) {
      ...
      for (const [change, status, error] of [
        [{ initiation_signature: "A".repeat(86) }, 401, "unauthorized"],
        [{ request_id: "A".repeat(42) + "B" }, 400, "bad_request"],
        [{ issued_at: "2026-02-30T12:00:00.000Z" }, 400, "bad_request"],
      ] as const) {
  ...
  it.each([2, 3])(
    "post-lock clock validation failure at sample %s is 503 without proof/authority/cleanup mutation",
    async (failAt) => {
      await truncateAll(ctx.db);
      const now = new Date();
      const p = await sharedAccounts(ctx.db, new Date(now.getTime() - 3000), [
        "shared-source-v1",
        "combat-v2",
      ]);
      expect(
      ...
      const c = await challenge(p.b);
      const retained = async () => ({
        devices: await ctx.db.select().from(fleetDevice).orderBy(fleetDevice.id),
        sessions: await ctx.db
          .select()
          .from(fleetDeviceSession)
          .orderBy(fleetDeviceSession.id),
        challenges: await ctx.db.select().from(fleetRecoveryChallenge),
        telemetry: await ctx.db.select().from(fleetTelemetryRow),
        leases: await ctx.db.select().from(fleetPublisherLease),
        audit: await ctx.db.select().from(auditLog).orderBy(auditLog.id),
      });
      const before = await retained();
      expect(before.telemetry).toHaveLength(1);
      expect(before.leases).toHaveLength(1);
      const validateClock = clock.fleetDatabaseNow;
      let samples = 0;
      const spy = vi.spyOn(clock, "fleetDatabaseNow").mockImplementation((tx) =>
        // Feed the sequential sample INTO real clock validation, never return an
        // Invalid Date past the validation boundary under test.
        validateClock(tx, ++samples === failAt ? new Date("invalid") : now),
      );
      try {
        const response = await complete(c);
        expect(samples).toBe(failAt);
  ...
  it.each(["55P03", "57014"])(
    ...
    async (code) => {
      ...
      expect(
        (await ctx.db.select().from(fleetRecoveryChallenge))[0].consumedAt,
      ).toBeNull();
      expect((await complete(c)).status).toBe(200);
    },
  );
  ...
  it("reports an outer transaction failure as a fixed transport refusal, never a proven result", async () => {
    ...
    expect((await ctx.db.select().from(fleetRecoveryChallenge))[0].consumedAt).toBeNull();
  });
  it("unwraps the closed reconnected DTO and the new credential works on the actual signed device route", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get("cache-control")).toBe("no-store");
    expect(response.headers.get("x-fleet-request-binding")).toMatch(/^[a-f0-9]{64}$/);
    ...
    expect(await replay.json()).toEqual({ protocol: 2, error: "unauthorized" });
    const fields = {
      protocol: 1 as const,
      method: "GET" as const,
  ...
  it("ignores caller Origin/Host for proof verification; the configured canonical origin is authoritative", async () => {
    ...
    expect(await refused.json()).toEqual({ protocol: 2, error: "unauthorized" });
    expect((await complete(c)).status).toBe(200);
  });
  it("exposes revocation only for a proven key; browser cookies and wrong signatures cannot serve as proof", async () => {
    ...
    expect(refused.status).toBe(401);
    expect(await refused.json()).toEqual({ protocol: 2, error: "unauthorized" });
    expect(await (await complete(c)).json()).toEqual({
      protocol: 2,
  ...
  it("uniformly refuses unknown/expired/consumed challenges and separately rejects malformed selectors", async () => {
    const c = await challenge();
    expect((await complete(c)).status).toBe(200);
    expect((await complete(c)).status).toBe(401);
    ...
    const body = { protocol: 2, nonce: c.nonce, recovery_signature: "A".repeat(86) };
    for (const id of [
      c.challenge_id,
      ...
      expect(response.status).toBe(id === "not-a-uuid" ? 404 : 401);
      expect(await response.json()).toEqual({
        protocol: 2,
        error: id === "not-a-uuid" ? "not_found" : "unauthorized",
      });
    }
  });
  it("uniformly gates disabled mode and bounds per-key requests without state disclosure", async () => {
    ...
    expect(await limited.json()).toEqual({ protocol: 2, error: "rate_limited" });
    ...
    expect(await completion.json()).toEqual({ protocol: 2, error: "feature_disabled" });
  });
    ...
    async (which) => {
      ...
      const body =
        which === "begin"
          ...
          : { protocol: 2, nonce: c.nonce, recovery_signature: "A".repeat(86) };
      ...
      for (const extra of [
        ...
        expect(await res.json()).toEqual({ protocol: 2, error: "bad_request" });
      }
      expect(await (await call(request(path, { ...body, protocol: 3 }))).json()).toEqual({
        protocol: 2,
        error: "update_required",
      });
      expect((await call(request(path + "?origin=forged", body))).status).toBe(400);
