...
import * as scheduler from "../src/worker/fleet-source-scheduler";
...
it("source flow preserves its body failure and drains the original handler after an early disposer fails, pools last", async () => {
  const events: string[] = [];
