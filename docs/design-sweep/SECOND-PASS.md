...
## Blocking the rest: this work is uncommitted

At the time of writing, the sweep's branch has made no commits — `HEAD` equals
the merge-base with `main`, and all 48 changed files are working-tree state in
`.claude/worktrees/design-sweep-2026-08-05`. That includes this file. Nothing
below survives a discarded worktree. Commit before relying on any of it.
...
| Both add-forms keep their values after a successful add, so a flat pool can be created twice | `payouts/[id]/flat-pool-form.tsx:33`, `add-participant-form.tsx:41` | Pre-existing behaviour, not introduced here; the false comment was fixed, the behaviour was not. Needs a `key` remount or a `ref` reset |
...
| `InlineEditField`'s `submitLabel`/`submitClassName`/`formClassName` have zero non-default call sites; its `Omit` leaves `value`/`onChange` assignable | `payouts/[id]/inline-edit-field.tsx:61-64,74` | Over-engineering findings need intent to judge |
