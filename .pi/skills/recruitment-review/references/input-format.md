...
Preparation runs first. Preparation-abort diagnostics are pipeline output, not model reports, and are not passed to the report checker. An unsafe or uninterpretable bundle emits that diagnostic without opening the report; it uses `Bundle: unavailable` when no validated identity is safely available. A valid model report exits 0, an invalid input or report exits 1, and invalid invocation syntax exits 2. Diagnostics are fixed and do not echo report contents.
