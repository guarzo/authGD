...
- **Severity:** moderate
- **Where:** `src/app/account/page.tsx:46, 484-497, 830-863`; `contact-state.tsx:182-198`
...
- **Prose out of the table cells.** The remediation prose for a faulted character now lives in a spanning `tr.drawer-row` sub-row directly under the row it names (page.tsx:830-863), not folded into the name cell — the single best decision on this page: it is why the manifest is a scannable table at all, and the comment on the sub-row records the measurement that ruled out the alternative (folding the remedy into the cell it describes forces a viewport-dependent row height, off-screen and unreachable at 390px). Do not "improve" this by folding the remedy back into the cell it describes.
