...
The checker accepts structurally valid completed and aborted reports for a successfully prepared bundle. A model-aborted report contains no citations at all. For completed reports, each of the five required sections must contain content. Coverage uses the rubric's seven non-empty labels for snapshot, character scope, dataset coverage, provenance, verification, synthetic-only state, and limitations/unexamined inputs; these labels only make the existing required content mechanically checkable. The checker also accepts the equivalent labels and dataset tables used by retained reports, without weakening the required content groups. Each `### Claim ...` block must contain the rubric's five non-empty labelled fields in order (plain or bold labels are accepted), an allowed assessment value, an interview citation for the applicant claim, and a record citation in Evidence unless the text explicitly states that no usable or applicable record exists. If there is no material checkable applicant claim, use the rubric's explicit no-claims statement instead of an empty or invented claim block. Material findings require all three non-empty subsections and citations for findings; an explicit `None identified...` statement and uncited coverage/provenance metadata observations remain valid. The follow-up section requires a citation grounding factual questions, while coverage-repair questions based only on packet metadata need no invented citation; alternatively, the section must explicitly state that no follow-up is needed. The bottom line must begin with exactly one permitted category and include an explanation.
