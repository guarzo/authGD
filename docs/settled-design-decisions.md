...
| **AMENDED 2026-08-07 (owner walkthrough, ruling R1)** — still exactly two sizes, but "in-row" is scoped by the density reason, not by the tag. A `Disclosure as="row"` drawer renders a `<tr>` and is nonetheless **not** in-row: one is open at a time, it spans the full width, nothing competes for the space. Drawer controls take 36px | Resolves a contradiction rather than reversing a decision: `e2e/sync.spec.ts`'s "a drawer's Re-run control sits at the standalone grade, not the in-row one" has been pinning the `/admin/sync` drawer to the standalone grade all along, and the "Full 36px" comment above the `Submit` in `payouts/[id]/notes-form.tsx` reasons the same way for a panel field. Only `/admin/accounts` read the row above as forbidding it, having inherited 28px by structural accident | `DESIGN.md` "Hit targets", `e2e/sync.spec.ts` (that test), `src/app/_components/note-form.tsx` |
